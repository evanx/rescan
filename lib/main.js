...
const getTypeCommand = command => {
    if (['del', 'type', 'ttl', 'none'].includes(command)) {
        return anyType;
    }
    if (command[0] === 'l') {
        return 'list';
...
const getCommand = () => {
    ...
    } else if (['hget'].includes(config.command)) {
        if (!config.field) {
            throw new Error('Command requires hashes field');
        }
    } else if (config.field) {
        throw new Error(`Field specified for invalid command '${command}'`);
    } else if (['expire'].includes(config.command)) {
        if (!config.ttl) {
            throw new Error(`Command requires TTL`);
        }
    } else if (config.ttl >= 0) {
        throw new Error(`TTL specified for invalid command '${command}'`);
    }
...
module.exports = async context => {
    ...
    if (config.field) {
        if (config.command !== 'hget') {
            throw new Error(`Expect 'hget' command when field specified`);
        }
    }
    if (config.ttl) {
        if (config.command !== 'expire') {
            throw new Error(`Expect 'expire' command when TTL specified`);
        }
    }
    while (true) {
        ...
        if (config.field) {
            ...
            keys.forEach((key, index) => {
                ...
                if (config.min >= 0 && ttl < config.min) {
                    ...
                    console.log([ttl, clc.cyan(key)].join(' '));
                }
            });
        } else {
